...
function getIframeHTML(src, link) {
    ...
    let html = `<div class="custom-code-d">`
        + `<iframe src="${src}" class="code-iframe-i" id=${id} />`
        + `<div class="custom-code-buttons" style="text-align: center;">`
            + `<button type="button" onclick="window.open('${link}', '_blank');" class="btn btn-outline-secondary">Voir le code source</button>`
            + `<button type="button" onclick="document.getElementById('${id}').src='${src}'" class="btn btn-outline-danger">Remettre à zéro</button>`
        + `</div>`
        + `</div>`
    ;
